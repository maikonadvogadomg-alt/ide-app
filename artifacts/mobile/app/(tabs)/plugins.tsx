...
function getInstallScope(cmd: string): { label: string; color: string; bg: string } {
  const c = cmd.toLowerCase();
  if (c.includes("pkg install") || c.includes("termux"))
    return { label: "📱 Assistida Termux", color: "#a78bfa", bg: "#1a0d3d" };
  if (c.includes("nix-env") || c.includes("nix profile"))
    return { label: "❄️  Nix (servidor)", color: "#7ec8e3", bg: "#001a2d" };
  if (c.includes("pip3") || c.includes("pip install"))
    return { label: "⚡ Servidor (pip)", color: "#00d4aa", bg: "#001a14" };
  if (c.includes("npm install") || c.includes("npx"))
    return { label: "⚡ Servidor (npm)", color: "#4ade80", bg: "#001a0a" };
  if (c.includes("cargo") || c.includes("rustup"))
    return { label: "⚡ Servidor (cargo)", color: "#f97316", bg: "#1a0800" };
  if (c.includes("http") && (c.includes("open") || c.includes("link")))
    return { label: "🔗 Externo (web)", color: "#60a5fa", bg: "#001233" };
  return { label: "⚡ Servidor local", color: "#00d4aa", bg: "#001a14" };
}
...
export default function PluginsScreen() {
  ...
  const [installedOutput, setInstalledOutput] = useState<string>("");
  ...
  const loadInstalledPackages = useCallback(async () => {
    if (!TERMINAL_API) {
      setInstalledOutput("❌ Servidor não configurado.\n\nConfigure o endereço do servidor em Configurações → Modo Termux.\nSem servidor, não é possível listar pacotes instalados.");
      return;
    }
    setLoadingInstalled(true);
    setInstalledOutput("⏳ Consultando servidor...\n");
    try {
      const cmd = [
        "echo '=== Node.js Global (npm list -g) ==='",
        ...
        "which node npm python3 pip3 git bash go rustc java php ruby lua dart flutter 2>/dev/null | sed 's|.*/||'",
        "echo ''",
        "echo '=== Versões ==='",
      ...
      const res = await fetch(`${TERMINAL_API}/exec`, {
        ...
        body: JSON.stringify({ command: cmd, sessionId: "installed_check" }),
      });
      if (!res.ok) {
        setInstalledOutput(`❌ Servidor retornou erro ${res.status}.\n\nVerifique se o servidor está rodando.`);
        return;
      }
      let out = "";
      if (Platform.OS === "web" || !res.body) {
        ...
        for (const line of text.split("\n")) {
          ...
          try {
            ...
            if (p.type === "stdout" || p.type === "stderr") out += p.data;
          } catch {}
        }
      } else {
        ...
        while (true) {
          ...
          for (const l of lines) {
            ...
            try {
              const p = JSON.parse(l.slice(6).trim());
              if (p.done) { reader.cancel(); break; }
              if (p.type === "stdout" || p.type === "stderr") out += p.data;
            } catch {}
          }
        }
      }
      setInstalledOutput(out.trim() || "Nenhum pacote encontrado.\n\nO servidor está online mas não retornou pacotes.");
    } catch (e: any) {
      setInstalledOutput(`❌ Erro ao conectar: ${e?.message ?? String(e)}\n\nVerifique a conexão com o servidor.`);
    } finally {
  ...
  // Quando não há servidor: mostra comando honestamente e copia para Termux
  const assistTermux = async (p: Plugin, v: PluginVersion) => {
    addLine(`⚠️  Servidor não disponível — instalação assistida via Termux`, "err");
    addLine(``, "out");
    addLine(`Este pacote precisa ser instalado no Termux (app separado).`, "info");
    addLine(`O comando abaixo foi COPIADO para a área de transferência:`, "info");
    addLine(``, "out");
    addLine(`$ ${v.cmd}`, "out");
    addLine(``, "out");
    addLine(`Passos:`, "info");
    addLine(`  1. Abra o Termux`, "info");
    addLine(`  2. Toque longo → Colar`, "info");
    addLine(`  3. Pressione Enter`, "info");
    addLine(``, "out");
    addLine(`⚠️  Nota: o pacote NÃO está instalado ainda.`, "err");
    addLine(`    Instale no Termux e volte aqui para confirmar.`, "err");
  ...
  const runInstall = async (p: Plugin, v: PluginVersion) => {
    ...
    try {
      ...
      if (!res.ok) {
        addLine(`\n⚠️ Servidor retornou erro HTTP ${res.status}.`, "err");
        addLine(`Usando instalação assistida via Termux:\n`, "info");
        await assistTermux(p, v);
        setInstallOk(false);
        setInstallDone(true);
        return;
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            addLine(`╔══════════════════════════════════════════════════╗`, "info");
            addLine(`║  INSTALANDO TODOS OS ${toInstall.length} PACOTES — AppIDE          ║`, "info");
            addLine(`╚══════════════════════════════════════════════════╝\n`, "info");
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {/* ── Botões Status + Instalados ── */}
      <View style={{ flexDirection: "row", marginHorizontal: 12, marginTop: 8, marginBottom: 4, gap: 8 }}>
        <TouchableOpacity
          onPress={() => { setShowStatus(true); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
          activeOpacity={0.82}
          style={{
            flex: 1, backgroundColor: "#0d1a0d", borderRadius: 12, borderWidth: 1, borderColor: "#1a4d1a",
            flexDirection: "row", alignItems: "center", paddingHorizontal: 12, paddingVertical: 11, gap: 8,
          }}
        >
          <Text style={{ fontSize: 16 }}>🩺</Text>
          <View style={{ flex: 1 }}>
            <Text style={{ color: "#4ade80", fontWeight: "700", fontSize: 13 }}>Diagnóstico</Text>
            <Text style={{ color: "#4a7a4a", fontSize: 10, marginTop: 1 }}>Node, Python, Git...</Text>
          </View>
          <Feather name="chevron-right" size={16} color="#4ade80" />
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => {
          ...
          activeOpacity={0.82}
          style={{
            flex: 1, backgroundColor: "#0d1a2d", borderRadius: 12, borderWidth: 1, borderColor: "#1a2d4d",
            flexDirection: "row", alignItems: "center", paddingHorizontal: 12, paddingVertical: 11, gap: 8,
          }}
        >
          <Text style={{ fontSize: 16 }}>📦</Text>
          <View style={{ flex: 1 }}>
            <Text style={{ color: "#60a5fa", fontWeight: "700", fontSize: 13 }}>Instalados</Text>
            <Text style={{ color: "#2d4a7a", fontSize: 10, marginTop: 1 }}>Ver pacotes reais</Text>
          </View>
              ...
              {versionPlugin?.versions.map((v) => (
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    {(() => {
                      const scope = getInstallScope(v.cmd);
                      return (
                        <View style={{ flexDirection: "row", alignItems: "center", marginTop: 5, gap: 6 }}>
                          <View style={{ paddingHorizontal: 7, paddingVertical: 2, borderRadius: 5, backgroundColor: scope.bg, borderWidth: 1, borderColor: scope.color + "66" }}>
                            <Text style={{ color: scope.color, fontSize: 10, fontWeight: "700" }}>{scope.label}</Text>
                          </View>
                        </View>
                      );
                    })()}
                    <Text style={{ color: "#555", fontSize: 10, fontFamily: Platform.OS === "ios" ? "Menlo" : "monospace", marginTop: 4 }} numberOfLines={1}>
                      $ {v.cmd.split("&&")[0].trim()}...
                    </Text>
                  </View>
      ...
      >
        <View style={[styles.installOverlay, { backgroundColor: "#0a0a0a" }]}>
          ...
          {/* Status */}
          <View style={[styles.statusBar, { backgroundColor: installDone ? (installOk ? "#10b98122" : "#1a0d2a") : "#007acc22", borderBottomColor: installDone ? (installOk ? "#10b981" : "#a78bfa") : "#007acc" }]}>
            <Text style={{ color: installDone ? (installOk ? "#10b981" : "#a78bfa") : "#007acc", fontSize: 12, fontWeight: "700" }}>
              {!installDone
                ? "⏳ Instalando no servidor... aguarde"
                : installOk
                ? "✅ Instalado no servidor com sucesso!"
                : "📱 Sem servidor — cole o comando no Termux para instalar no celular"}
            </Text>
          </View>
          ...
          {installDone && (
            <View style={[styles.installFooter, { borderTopColor: "#2a2a2a", paddingBottom: Math.max(insets.bottom, 16) + 8 }]}>
              ...
              {!installOk && installVersion?.cmd && (
                <TouchableOpacity
                  onPress={() => openInTermux(installVersion.cmd, installPlugin?.name ?? "")}
                  style={[styles.footerBtn, { backgroundColor: "#1a0d3d", borderWidth: 1, borderColor: "#a78bfa" }]}
                >
                  <Text style={{ fontSize: 15 }}>📱</Text>
                  <Text style={[styles.footerBtnText, { color: "#a78bfa" }]}>Copiar comando → Termux</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
      ...
      {/* Modal: O que está instalado */}
      <Modal
        visible={showInstalledModal}
        animationType="slide"
        onRequestClose={() => setShowInstalledModal(false)}
        statusBarTranslucent
      >
        <View style={{ flex: 1, backgroundColor: "#0a0a0a" }}>
          <View style={{ flexDirection: "row", alignItems: "center", paddingTop: topPadding + 12, paddingHorizontal: 16, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: "#1a1a2d", backgroundColor: "#0d0d1a" }}>
            <Text style={{ fontSize: 22 }}>📦</Text>
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={{ color: "#60a5fa", fontWeight: "800", fontSize: 17 }}>Pacotes Instalados</Text>
              <Text style={{ color: "#3a5a8a", fontSize: 11, marginTop: 1 }}>Consultado diretamente do servidor</Text>
            </View>
            <TouchableOpacity
              onPress={() => { setInstalledOutput(""); loadInstalledPackages(); }}
            ...
            <TouchableOpacity onPress={() => setShowInstalledModal(false)} style={{ padding: 8 }}>
              <Feather name="x" size={22} color="#888" />
            </TouchableOpacity>
          </View>
          {!TERMINAL_API && (
            <View style={{ margin: 16, padding: 16, backgroundColor: "#1a0d0d", borderRadius: 12, borderWidth: 1, borderColor: "#4d1a1a" }}>
              <Text style={{ color: "#f87171", fontWeight: "700", fontSize: 14, marginBottom: 6 }}>⚠️ Servidor não configurado</Text>
              <Text style={{ color: "#c0807a", fontSize: 13, lineHeight: 20 }}>
                Para ver os pacotes instalados, configure o endereço do servidor em:{"\n\n"}
                <Text style={{ color: "#fca5a5", fontWeight: "700" }}>Configurações → URL do Servidor / Modo Termux</Text>
              </Text>
            </View>
          )}
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 14 }}>
            <Text selectable style={{ color: "#d4d4d4", fontFamily: Platform.OS === "ios" ? "Menlo" : "monospace", fontSize: 12, lineHeight: 19 }}>
              {loadingInstalled ? "⏳ Carregando..." : installedOutput || "Pressione ↺ para atualizar."}
            </Text>
          </ScrollView>
          <View style={{ padding: 14, paddingBottom: Math.max(insets.bottom, 16) + 8, borderTopWidth: 1, borderTopColor: "#1a1a1a" }}>
            <TouchableOpacity
              onPress={() => setShowInstalledModal(false)}
              style={{ backgroundColor: "#1a1a1a", borderRadius: 12, padding: 14, alignItems: "center" }}
            >
              <Text style={{ color: "#888", fontWeight: "600", fontSize: 15 }}>Fechar</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
