...
export default function SettingsScreen() {
  ...
  const [expoTokenDraft, setExpoTokenDraft] = React.useState(settings.expoToken || "");
  const [showExpoToken, setShowExpoToken] = React.useState(false);
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: bottomPadding + 20 }}>
          ...
          <View style={{ padding: 14, gap: 14 }}>
            ...
            <View style={{ gap: 6 }}>
              ...
              <View style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
                <TextInput
                  value={expoTokenDraft}
                  onChangeText={setExpoTokenDraft}
                  placeholder="expo_token_..."
                  ...
                  style={{
                    ...
                    borderColor: expoTokenDraft ? colors.primary : colors.border,
                    borderWidth: 1, borderRadius: 8, padding: 10,
                    color: colors.foreground, fontSize: 13, fontFamily: "monospace",
                  }}
                />
                <TouchableOpacity onPress={() => setShowExpoToken(v => !v)} style={{ padding: 10 }}>
                  <Feather name={showExpoToken ? "eye-off" : "eye"} size={16} color={colors.mutedForeground} />
                </TouchableOpacity>
              </View>
              {expoTokenDraft.trim() !== (settings.expoToken || "") && (
                <TouchableOpacity
                  onPress={() => {
                    updateSettings({ expoToken: expoTokenDraft.trim() });
                    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                    Alert.alert("✅ Token salvo!", "Seu EXPO_TOKEN foi salvo. Agora o APK Builder usa este token automaticamente.");
                  }}
                  style={{ backgroundColor: colors.primary, borderRadius: 8, padding: 10, alignItems: "center" }}
                >
                  <Text style={{ color: "#fff", fontWeight: "700", fontSize: 13 }}>Salvar Token</Text>
                </TouchableOpacity>
              )}
              {settings.expoToken ? (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "#22c55e14", borderRadius: 8, padding: 8, borderWidth: 1, borderColor: "#22c55e33" }}>
                  <Feather name="check-circle" size={13} color="#22c55e" />
                  <Text style={{ color: "#22c55e", fontSize: 12 }}>Token salvo — APK Builder usa automaticamente</Text>
                </View>
              ) : null}
            </View>
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border, padding: 12 }]}>
          <Text style={{ color: colors.mutedForeground, fontSize: 12, marginBottom: 4, lineHeight: 18 }}>
            {"Chave gratuita do Google AI Studio — o app funciona standalone, sem precisar de nenhum servidor externo."}
          </Text>
          <TouchableOpacity
