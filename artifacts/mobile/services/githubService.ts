...
export async function cloneRepo(
  ...
  if (!repoRes.ok) {
    ...
    if (status === 403) {
      throw new Error(
        "Acesso negado (403). Possíveis causas:\n" +
        "• Repositório privado — adicione um token de acesso\n" +
        "• Limite de requisições do GitHub atingido — aguarde alguns minutos ou adicione um token\n" +
        "• Token inválido ou expirado"
      );
    }
    if (status === 404) {
      throw new Error(`Repositório "${owner}/${repo}" não encontrado. Verifique o URL.`);
    }
    throw new Error(`Erro ao acessar repositório (${status})`);
  }
  ...
  if (Platform.OS !== "web" && FileSystem.cacheDirectory) {
    ...
    if (dlResult.status !== 200) {
      ...
      if (dlResult.status === 403) {
        throw new Error(
          "Acesso negado ao baixar o ZIP (403).\n" +
          "• Para repositórios públicos grandes: adicione um token GitHub para aumentar o limite\n" +
          "• Para repositórios privados: token obrigatório\n" +
          "• Obtenha um token grátis em: github.com/settings/tokens"
        );
      }
      throw new Error(`Falha ao baixar repositório ZIP (${dlResult.status}). Verifique sua conexão.`);
    }
...
export async function dispatchWorkflow(
  ...
  if (!res.ok && res.status !== 204) {
    const err = await res.json().catch(() => ({}));
    throw new Error((err as any)?.message || `Falha ao disparar workflow (${res.status})`);
  }
}
