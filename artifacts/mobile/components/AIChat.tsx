...
async function fetchUrlContent(url: string): Promise<string> {
  ...
  // Remove HTML tags para leitura limpa — sem limite artificial
...
- TypeScript: mantenha tipagem consistente com o resto do projeto`;
    ...
    // Busca sempre funciona — DuckDuckGo/Unsplash direto do celular (sem servidor)
    if (imageQuery) {
  ...
  return (
    ...
    >
      ...
      {/* Banner: dicas do modo dev */}
      {chatMode === "dev" && messages.length === 0 && (
        <View style={{ backgroundColor: "#0d1117", borderBottomWidth: 1, borderBottomColor: "#21262d", paddingHorizontal: 10, paddingVertical: 4, flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Feather name="zap" size={10} color="#00d4aa" />
          <Text style={{ color: "#8b949e", fontSize: 10, flex: 1 }}>
            <Text style={{ color: "#e6edf3", fontWeight: "700" }}>IA Dev:</Text>
            {" "}cria arquivos, pesquisa web, acessa banco de dados.
          </Text>
        </View>
      )}
      ...
      {chatMode === "dev" && activeProject && !isLoading && (
        <ScrollView
          ...
          contentContainerStyle={{ paddingHorizontal: 8, paddingVertical: 3, gap: 4 }}
...
const styles = StyleSheet.create({
  ...
  suggestionChipText: { fontSize: 11, fontWeight: "500" },
  quickReplyBar: {
  ...
  quickReplyText: { fontSize: 11, fontWeight: "600" },
  inputRow: {
    flexDirection: "row",
    alignItems: "flex-end",
    padding: 10,
